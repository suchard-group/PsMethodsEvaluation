...
#
# This file is part of LegendT2dmTestCases
#
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "LegendT2dmTestCases")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "LegendT2dmTestCases")
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "LegendT2dmTestCases")
