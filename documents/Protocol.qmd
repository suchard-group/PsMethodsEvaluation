...
format: 
  ...
  pdf:
    ...
    header-includes:
      ...
      - \newcommand{\footerDate}{Jan 28, 2024}
      - \input{header.tex}
    template-partials: 
      - before-body.tex
...
tab <- kbl(parties, booktabs = TRUE, linesep = "") %>%
...
```{r}
#| label: tbl-1
#| tbl-cap: "New-user cohort method analysis variants of propensity score (PS) adjustment of outcome model, its conditioning and patient usage. LSPS stands for large-scale propensity score model, utilizing the large set of generic characteristics. SSPS stands for small-scale propensity score model, which uses the set of 50 hand-picked covariates"
table1
```
...
tab <- kbl(data_sources, booktabs = TRUE, linesep = "") %>%
...
# Creating results table kable objects
...
<!-- HTML figures -->
```{r, echo = F, message = F, out.width = "5.5in"}
...
if(knitr::is_html_output()){
  ...
  kable_styling(bootstrap_options = c("striped"),
    ...
    column_spec(7, "0.7in") %>%
    column_spec(8, "0.7in") %>%
  as_image(file = "./figures/metrics.png")
}
...
if(knitr::is_html_output()){
  ...
  kable_styling(bootstrap_options = c("striped"),
    ...
    column_spec(7, "0.7in") %>%
    column_spec(8, "0.7in") %>%
  as_image(file = "./figures/metricsCali.png")
}
...
Occurring between -365 days and 0 days relative to index:
...
Occurring between -30 days and 0 days relative to index:
